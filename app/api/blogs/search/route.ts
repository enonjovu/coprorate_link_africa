...
export async function GET(req:Request ,res:Response)
{
    try{
        ...
        const createTextIndex = async (collection:any)=> {
            await collection.createIndex({ title: "text", story: "text" });
        }

        // Call this function before using full-text search
        await createTextIndex(collection);


        // Fetch the data
        const results = await collection
        .find({ $text: { $search: searchQuery? searchQuery : ""} })
        .toArray();
