...
function ProfileCard({ profile }: { profile: IndividiualProfileDocument }) {
    return (
        <div className="relative flex max-w-sm flex-col overflow-hidden rounded-xl shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            <a href={`/leaders/${profile.id}`} className="absolute left-0 top-0 z-20 h-full w-full ">
                &nbsp;
            </a>
            <div className="h-auto overflow-hidden">
                <div className="relative h-44 overflow-hidden">
                    <img src={profile.profile_image[0].url} alt={profile.name} />
                </div>
            </div>
