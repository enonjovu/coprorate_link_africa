import { deleteSideAds, fetchSideAds } from "@/app/action";
...
const SideAdsListingComponent = async ({ page }: { page: string }) => {
    const result = await fetchSideAds(page);
    ...
    async function handleDelete(data: FormData) {
        ...
        if (itemId != null) {
            try {
                const res = await deleteSideAds(itemId);
                if (res) {
...
export default SideAdsListingComponent;