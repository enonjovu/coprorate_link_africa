...
const TenderListingComponent = async ({ page }: { page: string }) => {
    const result = await fetchTenders(page);
    const { tenders, itemsCount } = result;
    ...
    async function handleDelete(data: FormData) {
        ...
        if (itemId != null) {
            try {
                const res = await deleteTender(itemId);
                if (res) {
                    ...
                    <div className="flex flex-row flex-wrap -mx-3">
                        {
                            tenders.length > 0 ?
                                tenders.map(tender => (
                                    <div key={tender._id} className="flex-shrink max-w-full w-full sm:w-1/3 px-3 pb-3 pt-3 sm:pt-0 border-b-2 sm:border-b-0 border-dotted border-gray-100">
                                        <div className="flex flex-row sm:block hover-img">
                                            <Image width={900} height={800} className="max-w-full w-full h-40 min-h-40 max-h-40 overflow-hidden object-cover mx-auto" src={tender.company.logo[0].url} alt="alt title" />
                                            ...
                                            <div className="w-full h-16 bg-red-700 roundeg-lg flex items-center justify-center space-x-4">
                                                ...
                                                <form action={handleDelete}>
                                                    <input name="itemId" className="hidden" value={tender._id} />
                                                    <button type="submit" className="cursor-pointer rounded-lg border-[1px] border-white p-2">
                                ...
                                <h1 className="text-2xl font-bold text-center w-full">No tenders Found</h1>
                        }
                    </div>
                </div>
                {
...
export default TenderListingComponent;