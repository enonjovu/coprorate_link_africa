                    ...
                    <div className="flex flex-row flex-wrap -mx-3">
                        {
                            companies.length > 0 ?
                                companies.map(company => (
                                    <div key={company._id} className="flex-shrink max-w-full w-full sm:w-1/3 px-3 pb-3 pt-3 sm:pt-0 border-b-2 sm:border-b-0 border-dotted border-gray-100">
                                        <div className="flex flex-row sm:block hover-img">
                                            <Image width={900} height={800} className="max-w-full w-full h-40 min-h-40 max-h-40 overflow-hidden object-cover mx-auto" src={company.logo[0].url} alt="alt title" />
                                            <h1 className="text-center text-lg">{company.name}</h1>
                                            <div className="w-full h-16 bg-red-700 roundeg-lg flex items-center justify-center space-x-4">
                                ...
                                <h1 className="text-2xl font-bold text-center w-full">No companies Found</h1>
                        }
                    </div>
                </div>
                {
                            ...
                            <nav aria-label="Page navigation">
                                <ul className="flex justify-center items-center space-x-0">
                                    <li>
                                        {
                                            currentPage === 1 ? (null)
                                                : <Link className="block relative py-3 px-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:text-gray-100 hover:bg-gray-700 dark:hover:bg-gray-700 -mr-0.5 rounded-r" href={`/admin/directories/?page=${prevPage}`} aria-label="Previous">
...

                                    {
                                        pageNumbers.map((pageNo, index) => (
                                            <li key={index}>
                                                <Link className="block relative py-3 px-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:text-gray-100 hover:bg-gray-700 dark:hover:bg-gray-700 -mr-0.5" href={`/admin/directories/?page=${pageNo}`}>
                                                    {pageNo}
                                                </Link>
                                            </li>
                                        ))
                                    ...
                                    <li>
                                        {
                                            currentPage === totalPages ? (null)
                                                : <Link className="block relative py-3 px-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:text-gray-100 hover:bg-gray-700 dark:hover:bg-gray-700 -mr-0.5 rounded-r" href={`/admin/directories/?page=${nextPage}`} aria-label="Next">
                                                    <span aria-hidden="true">»</span>
                                                </Link>
                                        }
                                    </li>
